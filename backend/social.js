...
async function request(path, options = {}, timeoutMs = 20000) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(body?.error || body?.message || `Social request failed (${response.status})`);
    return body;
  } finally {
...
export async function fetchSocialGraph(userId, viewerId = userId) {
